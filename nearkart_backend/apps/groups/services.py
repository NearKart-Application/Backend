...
class GroupService:
    ...
    @staticmethod
    def get_member(group, user):
        try:
            return GroupMember.objects.select_related('user').get(group=group, user=user)
    ...
    def add_member(group: Group, user_to_add) -> GroupMember:
        # Vendor groups: user must follow the store
    ...
    def remove_member(group: Group, user_to_remove):
        ...
        if member.role == GroupMemberRole.ADMIN and group.created_by_id == user_to_remove.id:
    ...
    def get_shared_products(group: Group):
        return (
            ...
            .filter(group=group)
            .select_related('product', 'shared_by', 'finalized_by')

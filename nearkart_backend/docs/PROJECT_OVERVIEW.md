...
| S27 | Store Click Tracking — wire `store_views` (HyperLogLog 30-day sum) · `inquiries_pending` (unread conversations) · `products_need_action` (zero-stock active products) into vendor stats endpoint | Done ✅ | `apps/stores/views.py` · `docs/sprint_27_store_click_tracking/` |
| S28 | Product Demo Video (`video_type` + `product` FK on `Video` · `GET products/{id}/demo-video/`) · Voice OTP (`delivery_method: voice` → Twilio TwiML call · `send_otp_voice` Celery task) | Done ✅ | `apps/videos/` · `apps/products/` · `apps/auth_app/` · `docs/sprint_28_localization_video_voice/` |
| S29 | Backend Performance Hardening · Bug Fixes — N+1 elimination (store subcategories, product variants, following feed) · pagination on 8 unbounded list endpoints · bulk Celery task writes · 4 DB index migrations · `CONN_MAX_AGE=0` production fix · 3 runtime bug fixes (NameError review reply, AttributeError PDF export, wrong review notification args) | Done ✅ | cross-app · `config/settings/production.py` · `docs/sprint_29_performance_bugfix/` |
